...
def readJson(filenames):

    json_str = ""
    for foil in filenames:
        with open(foil) as json_file:
            json_str= json_str +json_file.read()
...
def createIndex(json_data, out_file):
...
def run(ls):
    ...
    json_data = readJson(midSource)
    #json_data = json_data["wikidump"]
    
    createIndex(json_data, dest)
